...
from ..prompts import self_discover as self_discover_prompts
from ..utils import ui_utils 
from ..utils.prompt_utils import parse_question_data 
from .. import config
from ..prompts.self_discover import format_self_discover_prompt, ESSAY_SELF_DISCOVER_PROMPT_TEMPLATE
from ..utils.text_utils import clean_llm_answer_for_similarity

logger = logging.getLogger(__name__)

def run_self_discover_strategy(
    parsed_questions: List[Dict[str, Any]],
    model_config: Dict[str, Any],
    reasoning_prompt_template: str,
    format_reasoning_prompt_func: Callable,
    processing_animation: ui_utils.LoadingAnimation,
    current_model_config_id: str
) -> Dict[str, Any]:
    """
    Runs the Self-Discover strategy adapted for essay generation.

    Args:
        parsed_questions: List of parsed question data.
        model_config: Configuration for the LLM.
        reasoning_prompt_template: The prompt template for the reasoning/discovery phase.
        format_reasoning_prompt_func: Function to format the reasoning prompt.
    ...
    for i, question_data in enumerate(parsed_questions):
        if processing_animation and hasattr(processing_animation, 'message'):
            processing_animation.message = f"Processing with {current_model_config_id} (Self-Discover Essay): {i+1}/{num_questions} questions"        
        
        prompt = format_reasoning_prompt_func(
            question_data=question_data, 
            prompt_template_str=reasoning_prompt_template, 
            use_essay_format=True 
        )
        ...
        try:
            ...
            if llm_response_data and not llm_response_data.get("error_message"):
                ...
                logger.error(f"Error with {model_config.get('config_id')} (Self-Discover) for question {question_data.get('question_id', i)}: {error_message}")
                result_item.update({
            ...
            logger.error(f"Strategy level exception for {model_config.get('config_id')} (Self-Discover) question {question_data.get('question_id', i)}: {e}", exc_info=True)
            result_item.update({
